...
function getQuadBezierPoint(x0: number, y0: number, cx: number, cy: number, x1: number, y1: number, s: number) {
...
export function generateHeroSvg(frameIndex: number = -1, totalFrames: number = 36): string {
  ...
  if (r1Progress < 0.5) {
    ship1 = getQuadBezierPoint(150, 230, 180, 280, 210, 300, r1Progress * 2);
  } else {
    ship1 = getQuadBezierPoint(210, 300, 280, 250, 380, 160, (r1Progress - 0.5) * 2);
  }

  // Route 2: Singapore (210, 300) -> Cape of Good Hope (310, 350) -> Rotterdam (380, 160)
  const r2Progress = (t + 0.6) % 1;
  let ship2 = { x: 210, y: 300 };
  if (r2Progress < 0.5) {
    ship2 = getQuadBezierPoint(210, 300, 260, 360, 310, 350, r2Progress * 2);
  } else {
    ship2 = getQuadBezierPoint(310, 350, 360, 280, 380, 160, (r2Progress - 0.5) * 2);
  }

  // Route 3: Shanghai (150, 230) -> Pacific -> US LA (490, 180) -> Mexico Monterrey (510, 240)
  const r3Progress = (t + 0.35) % 1;
  let ship3 = { x: 150, y: 230 };
  if (r3Progress < 0.7) {
    ship3 = getQuadBezierPoint(150, 230, 320, 110, 490, 180, r3Progress / 0.7);
  } else {
    const s = (r3Progress - 0.7) / 0.3;
    ship3 = { x: 490 + (510 - 490) * s, y: 180 + (240 - 180) * s };
  }

  // Radar wave expansion (0 to 1)
  const radar1 = (t * 2) % 1;
  const radar2 = ((t * 2) + 0.5) % 1;
  const radarRadius1 = 8 + radar1 * 25;
  const radarOpacity1 = Math.max(0, 1 - radar1);
  const radarRadius2 = 8 + radar2 * 25;
  const radarOpacity2 = Math.max(0, 1 - radar2);
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 420" width="1200" height="420">
  <defs>
    ...
    <!-- Text Gradients -->
    <linearGradient id="cyan-text" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#38BDF8" />
      <stop offset="100%" stop-color="#0EA5E9" />
    </linearGradient>

    <linearGradient id="emerald-text" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#34D399" />
      <stop offset="100%" stop-color="#10B981" />
    </linearGradient>

    <linearGradient id="violet-text" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%" stop-color="#A78BFA" />
      <stop offset="100%" stop-color="#8B5CF6" />
    </linearGradient>

    <!-- Ambient Glow Radials -->
    <radialGradient id="glow-cyan" cx="25%" cy="35%" r="50%">
      <stop offset="0%" stop-color="#0EA5E9" stop-opacity="0.25" />
      <stop offset="100%" stop-color="#0EA5E9" stop-opacity="0" />
    </radialGradient>
    <radialGradient id="glow-violet" cx="80%" cy="30%" r="50%">
      <stop offset="0%" stop-color="#8B5CF6" stop-opacity="0.2" />
      <stop offset="100%" stop-color="#8B5CF6" stop-opacity="0" />
    </radialGradient>

    <!-- Grid Pattern -->
    <pattern id="hex-grid" width="36" height="36" patternUnits="userSpaceOnUse">
      <path d="M 36 0 L 0 0 0 36" fill="none" stroke="#334155" stroke-width="0.6" opacity="0.4" />
      <circle cx="0" cy="0" r="1" fill="#475569" opacity="0.6" />
    </pattern>
  ...
  <circle cx="300" cy="150" r="380" fill="url(#glow-cyan)" />
  <circle cx="950" cy="150" r="400" fill="url(#glow-violet)" />
  <rect width="1200" height="420" fill="url(#hex-grid)" />

  <!-- 2. LEFT SIDE: Global Maritime Trade Route Map (X: 30 to 580) -->
  <g id="maritime-map" transform="translate(10, 0)">
    <!-- Stylized Continents / Regional Clusters (Subtle Background Polygons) -->
    <path d="M 100 170 L 160 160 L 210 200 L 180 260 L 130 250 Z" fill="#1E293B" stroke="#334155" stroke-width="1" opacity="0.5" />
    <path d="M 320 110 L 420 100 L 440 170 L 370 200 L 330 160 Z" fill="#1E293B" stroke="#334155" stroke-width="1" opacity="0.5" />
    <path d="M 270 230 L 350 240 L 360 360 L 300 370 L 260 300 Z" fill="#1E293B" stroke="#334155" stroke-width="1" opacity="0.5" />
    <path d="M 450 140 L 560 130 L 570 260 L 480 270 Z" fill="#1E293B" stroke="#334155" stroke-width="1" opacity="0.5" />
    ...
    <!-- Node 1: Shanghai -->
    <circle cx="150" cy="230" r="6" fill="#0EA5E9" stroke="#FFFFFF" stroke-width="2" filter="url(#glow)" />
    <text x="150" y="215" font-family="system-ui, -apple-system, sans-serif" font-size="11" font-weight="700" fill="#E2E8F0" text-anchor="middle">CNSHA (Shanghai)</text>

    <!-- Node 2: Singapore -->
    <circle cx="210" cy="300" r="5" fill="#38BDF8" stroke="#FFFFFF" stroke-width="1.5" />
    <text x="210" y="318" font-family="system-ui, -apple-system, sans-serif" font-size="10" font-weight="600" fill="#94A3B8" text-anchor="middle">SGSIN (Malacca)</text>
    ...
    <rect x="250" y="365" width="120" height="20" rx="4" fill="#451A03" stroke="#F59E0B" stroke-width="1" opacity="0.9" />
    <text x="310" y="379" font-family="system-ui, -apple-system, sans-serif" font-size="10" font-weight="700" fill="#FDE68A" text-anchor="middle">CAPE DETOUR (+12d)</text>

    <!-- Node 5: Rotterdam -->
    <circle cx="380" cy="160" r="6" fill="#38BDF8" stroke="#FFFFFF" stroke-width="2" filter="url(#glow)" />
    <text x="380" y="145" font-family="system-ui, -apple-system, sans-serif" font-size="11" font-weight="700" fill="#E2E8F0" text-anchor="middle">NLRTM (Rotterdam)</text>

    <!-- Node 6: US LAX -->
    <circle cx="490" cy="180" r="5" fill="#10B981" stroke="#FFFFFF" stroke-width="1.5" />
    <text x="490" y="165" font-family="system-ui, -apple-system, sans-serif" font-size="10" font-weight="600" fill="#94A3B8" text-anchor="middle">USLAX (LA)</text>
  ...
  <g id="dashboard-header" transform="translate(590, 0)">
    ...
    <!-- Main Title -->
    <text x="0" y="100" font-family="system-ui, -apple-system, sans-serif" font-size="32" font-weight="900" fill="#FFFFFF" letter-spacing="1">GLOBAL SUPPLY CHAIN &amp;</text>
    <text x="0" y="138" font-family="system-ui, -apple-system, sans-serif" font-size="32" font-weight="900" fill="url(#cyan-text)" letter-spacing="1">LOGISTICS INTELLIGENCE</text>
...
async function main() {
  console.log("=== Generating Supply Chain Research Hero Banner ===");
  ...
  console.log("⏳ Rendering GIF frames with @resvg/resvg-js...");
  const tmpDir = path.join(process.cwd(), "tmp_frames");
  if (!fs.existsSync(tmpDir)) {
    fs.mkdirSync(tmpDir, { recursive: true });
  }

  const totalFrames = 30; // 30 frames at 15fps = 2.0s smooth loop
  for (let i = 0; i < totalFrames; i++) {
    const frameSvg = generateHeroSvg(i, totalFrames);
    ...
    fs.writeFileSync(path.join(tmpDir, `frame_${frameNumber}.png`), pngData);
  }
